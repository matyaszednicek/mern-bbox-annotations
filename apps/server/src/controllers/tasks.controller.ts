...
import { AddImagesToTaskInput, CreateTaskInput } from '../schemas/tasks.schema';
...
export const getTask: RequestHandler<{ taskId: string }> = async (
  req,
  res,
  next
) => {
...
> = async (req, res, next) => {
  try {
    ...
    const newTask = await createNewTaskForUser(req.identity.id, labels);
